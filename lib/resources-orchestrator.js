...
module.exports = {
  ...
  async initialSetup(ringpopInstance, resources, cacheAllResources = false) {
    ...
    await Promise.all(resourcesForThisHashring
      .map(resource => handleNewResources(resource, resourceHandlerDefinition, log)));
  },
  /**
   *
   * @param {Ringpop} ringpopInstance
  ...
  async rebalanceResources(ringpopInstance) {
    ...
    const resourcesToBeDeallocated = resourcesHandledByThisHashring
      .filter(resource => !_.find(newResourcesForThisHashring, { id: resource.id }));
    const resourcesToBeAllocated = newResourcesForThisHashring
      .filter(resource => !_.find(resourcesHandledByThisHashring, { id: resource.id }));
    ...
    const newResourcesPromises = resourcesToBeAllocated
      .map(resource => handleNewResources(resource, resourceHandlerDefinition, log));
    const oldResourcesPromises = resourcesToBeDeallocated
      .map(resource => handleResourceTermination(resource, resourceHandlerDefinition, log));
