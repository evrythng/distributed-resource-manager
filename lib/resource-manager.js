...
const resourceManagerApi = {
  ...
  async stop() {
    ...
    await Promise.all(resourcesOrchestrator
      .getResourcesHandledByThisHashring()
      .map(resourcesOrchestrator.handleResourceTermination));
...
module.exports = async (config) => {
  ...
  try {
    const resourcesRetrieverConfig = config.resourcesRetrieverConfig ?
      config.resourcesRetrieverConfig :
      config.fetchResourcesHttpOptions;

    await resourcesRetriever.setupResourcesRetriever(
      resourcesRetrieverPluginSpecified,
      resourcesRetrieverConfig,
      resourceManagerApi,
    );
